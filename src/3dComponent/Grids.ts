...
export class Grids {
  ...
  set h_length(length: number) {
    ...
    for (let tick of this.v_ticks) {
      const p1 = new Vector3(-this._max_h_lenght / 2, tick, 0);
      const p2 = new Vector3(-this._max_h_lenght / 2 + this._h_lenght, tick, 0);
  ...
  set v_length(length: number) {
    ...
    for (let tick of this.h_ticks) {
      const p1 = new Vector3(tick, this._max_v_lenght / 2, 0);
      const p2 = new Vector3(tick, this._max_v_lenght / 2 - this._v_lenght, 0);
  ...
  createMeshes() {
    const h_lines: Line[] = [];
    for (let tick of this.v_ticks) {
      const p1 = new Vector3(-this._h_lenght / 2, tick, 0);
      const p2 = new Vector3(this._h_lenght / 2, tick, 0);
      h_lines.push(
        new Line({
          p1,
          p2,
